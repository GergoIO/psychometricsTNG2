...
#' 
#' @description fnHistogramScore creates a histogram of scores. Optional arguments draw a boxplot above the histogram and shade the plot background according to supplied grade boundaries.
#' The output from fnHistogramScore is a ggplot object which can then be saved or customised as needed.
#' See Examples for more details.
#' 
#' @usage fnHistogramScore(x,lo=0,hi=100,gradeScheme="",gradeBounds=c(),main="",ylab="Frequency (N students)",plotBox=TRUE,pcScore=TRUE)
#' 
...
#' 	fnHistogramScore(x=rnorm(86,30,10),lo=-10,hi=80,gradeScheme="UBSE",gradeBounds=c(20,25,45))
#' 	
#' Ability to accommodate NA values and to adjust lo and hi (set to 0 and 80 in this example) to fit data
#' 	fnHistogramScore(x=c(-1,2,rnorm(86,30,10),65,93,100,NA),lo=0,hi=80,gradeScheme="USE",gradeBounds=c(18.25,45.62))
#' 	
#' If length(gradeBounds) not equal to nchar(gradeScheme) - 1 then no background shading is carried out
#' 	fnHistogramScore(x=rnorm(86,30,10),gradeScheme="FP",gradeBounds=c(18.25,25.62))
#' 	
#' Scores need not be percentages and the boxplot can be removed
#' 	fnHistogramScore(x=rnorm(86,150,20),lo=100,hi=200,gradeScheme="FP",gradeBounds=c(135),plotBox=FALSE,pcScore=FALSE)
#' 	
#' Outputs a ggplot object, which can them be edited as necessary.
#' 	plotExample<-fnHistogramScore(x=rnorm(86,50,15),gradeScheme="US",gradeBounds=c(44))
...
#' 
#' @export
fnHistogramScore<-function(x,lo=0,hi=100,gradeScheme="",gradeBounds=c(),main="",ylab="Frequency (N students)",plotBox=TRUE,pcScore=TRUE){
    # Written by: Martin Roberts
    # Last updated: 16/01/2018
    ################################################
    x<-na.omit(x)
    if(min(c(x,gradeBounds),na.rm=TRUE)<lo) {lo<-10*floor(min(c(x,gradeBounds),na.rm=TRUE)/10)}	# If min(c(x,gradeBounds))<lo then lo is adjusted accordingly (in multiples of 10)
    if(max(c(x,gradeBounds),na.rm=TRUE)>hi) {hi<-10*ceiling(max(c(x,gradeBounds),na.rm=TRUE)/10)}	# If max(c(x,gradeBounds))>hi then hi is adjusted accordingly (in multiples of 10)
    ...
    ggplot() + 
        ...
        geom_histogram(aes(x),breaks=seq(lo,hi+1,1),col="black",fill=ifelse(plotGrades,"grey40","maroon"),na.rm=TRUE,closed="right") +
